...
#   type                = any
#   description         = "If used, the network module used to provision the underlying VCN.  If this is provided, there's no need to provide vcn_id or vcn_cidr."
#   default             = null
# }
variable "vcn_id" {
  type        = string
...
variable "nsg_ids_to_associate" {
  type        = list(string)
  description = "The OCIDs of the NSGs that should be associated with the bastion vNIC."
  default     = null
}

variable "ssh_src_cidrs" {
  type        = list(string)
  description = "The CIDRs that are permitted inbound to this instance via SSH (tcp/22), added as NSG rules to the bastion NSG."
  default     = []
}
variable "ssh_dst_cidrs" {
  type        = list(string)
  description = "The CIDRs that this bastion is permitted to connect to via SSH (tcp/22), added as NSG rules to the bastion NSG."
  default     = []
}

variable "ssh_src_nsg_ids" {
  type        = list(string)
  description = "The NSG OCIDs that are permitted inbound to this instance via SSH (tcp/22), added as NSG rules to the bastion NSG."
  default     = []
}
variable "ssh_dst_nsg_ids" {
  type        = list(string)
  description = "The NSG OCIDs that this bastion is permitted to connect to via SSH (tcp/22), added as NSG rules to the bastion NSG."
  default     = []
}
...
variable "create_compute" {
  type        = bool
  description = "Whether or not the compute should be created for the bastion."
  default     = true
}
